(function() {
	...
	GaReportExt.prototype.grantAccess = function() {
		googleChromeExtAuth.login();
	};
				...
				if (data) {
					$(data).find("feed").each(function() {
						...
						feed.children("entry").each(function() {
							...
							entryData.tableId = entry.children(":last").text();
				...
				.datepicker(
						{
							...
							onSelect : function(selectedDate) {
								var option = this.id == "startDate" ? "minDate" : "maxDate", instance = $(this)
										.data("datepicker"), date = $.datepicker.parseDate(instance.settings.dateFormat
	...
	function retrieveData(tableId, startDate, endDate, byChannel) {
		...
		setTimeout(function() {
			getCompanyRef(tableId, $("#organic_company_referral").val().replace(/,/g, "|"), startDate, endDate, channel);
		}, 200);
		// get direct
		setTimeout(function() {
			...
			googleAnalyticsExt.getMetrics({
				...
				success : function(data) {
					if (channel) {
						...
						allResult.organic.search[channel.code] = showResultForChannel(data, channel, "#organic_search_result");
						...
						allResult.organic.search = showResult(data, "#organic_search_result", [ 6, 'Organic', 'Search' ]);
			...
			googleAnalyticsExt.getMetrics({
				...
				success : function(data) {
					if (channel) {
						...
						allResult.organic.inref[channel.code] = showResultForChannel(data, channel, "#organic_internal_result");
			...
			googleAnalyticsExt.getMetrics({
				...
				success : function(data) {
					if (channel) {
						...
						allResult.organic.comref[channel.code] = showResultForChannel(data, channel, "#organic_company_result");
		...
		googleAnalyticsExt.getMetrics({
			...
			success : function(data) {
				if (channel) {
					...
					allResult.organic.direct[channel.code] = showResultForChannel(data, channel, "#organic_direct_result");
			...
			googleAnalyticsExt.getMetrics({
				...
				success : function(data) {
					if (channel) {
						...
						allResult.organic.syn[channel.code] = showResultForChannel(data, channel, "#organic_syndication_result");
						chartData.organic.syn = chartData.organic.syn ? chartData.organic.syn : {};
						chartData.organic.syn[channel.code] = getEntries(data);
					} else {
						allResult.organic.syn = showResult(data, "#organic_syndication_result", [ 0, 'Organic', 'Syndication' ]);
					...
					.getMetrics({
						...
						success : function(data) {
							if (channel) {
								...
								allResult.mmb.nav[channel.code] = showResultForChannel(data, channel, "#mmb_navigation_result");
								chartData.mmb.nav = chartData.mmb.nav ? chartData.mmb.nav : {};
								chartData.mmb.nav[channel.code] = getEntries(data);
							} else {
								allResult.mmb.nav = showResult(data, "#mmb_navigation_result", [ 0, 'Marketing Media-buy',
										'Navigation' ]);
	...
	function showResultForChannel(data, channel, containerSelector) {
		...
		var result = renderResults(getAggregates(data), containerSelector + channel.code, "#channel_result_tmpl", null, 0, hasSub);
		...
		if (siteConfig) {
			...
			for ( var i = 0; i < channels.length; i++) {
				...
				$("#site_structure").append(
						"<option value='" + site.version.num + "' " + (i == 0 ? "selected" : "") + " >" + site.version.desc
								+ "</option>");
			}
		}
	}
	...
	function createCharts(category, item) {
		if ($("#dataDivider").val() == "ga:date") {
			...
			createLineChart("pv_visit_charts", "Pages per Visit trend chart", "page per visit", category, item, series.pv_visit);
			createLineChart("bounce_rate_charts", "Bounce Rate trend chart", "bounce rate", category, item, series.bounce_rate);
			...
			formatter = function() {
				...
				if (points) {
					...
					return new Date(this.x).toLocaleDateString() + "<br/>" + this.series.name + ': ' + convertTimeToStr(this.y);
				}
			};
	...
	function buildSerieData(entries, serieName, series) {
		if (entries) {
			...
			for ( var j = 0; j < entries.length; j++) {
				...
				pvVisitSerie.data.push(Math.round(entry.visits == 0 ? 0 : parseFloat(entry.pageviews) / parseFloat(entry.visits)
						* 100) / 100.0);
				bounceRateSerie.data.push(Math.round(entry.visits == 0 ? 0 : parseFloat(entry.bounces) / parseFloat(entry.visits)
						* 100));
